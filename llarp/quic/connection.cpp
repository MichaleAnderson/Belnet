...
namespace llarp::quic
{
  ...
  {
    return o << bmq::to_hex(c.data, c.data + c.datalen);
  }
        ...
        ngtcp2_conn* conn_, ngtcp2_cid* cid_, uint8_t* token, size_t cidlen, void* user_data)
    {
      ...
      *cid_ = cid;
      ...
      LogDebug(
          ...
          bmq::to_hex(token, token + NGTCP2_STATELESS_RESET_TOKENLEN));
...
#pragma GCC diagnostic pop
  ...
  Connection::recv_transport_params(std::basic_string_view<uint8_t> data)
  {
    ...
    try
    {
      bmq::bt_dict_consumer meta{belnet_metadata};
    ...
    catch (const bmq::bt_deserialize_invalid& c)
    {
  ...
  Connection::send_transport_params(ngtcp2_crypto_level level)
  {
    ...
    auto* bufend = buf + conn_buffer.size();
    {
      ...
      std::string belnet_metadata = bt_serialize(bmq::bt_dict{
          {"#", tunnel_port},
      });
